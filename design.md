...
- **`axe-core`**: `.github/workflows/accessibility.yml`, semanal + manual, corre `@axe-core/cli` contra las URLs de producción (mismo patrón que `lighthouse.yml`) — informativo (`continue-on-error: true`), no bloquea. No se pudo probar en local (Mac): `@axe-core/cli` usa Selenium + chromedriver, y la descarga del binario de chromedriver falló en esta máquina (`ENOENT`); se decidió confiar en que el runner de GitHub Actions (Ubuntu, Chrome preinstalado) sí lo resuelve — revisar el primer run si algo no cuadra.
