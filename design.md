...
users/{userId}/seen/{movieId}
  tmdbId, title, posterPath, releaseYear, voteAverage, watchedAt, genreIds?

users/{userId}/notifications/{notificationId}
  type, movieId, movieTitle, posterPath, personId, personName, releaseDate, read, createdAt
```
...
- **Unificación de "vista" (2026-09-14)**: `SeenMovie` (`users/{userId}/seen/{movieId}`) y `WatchedMovie` (`followedPeople/{personId}/watchedMovies/{movieId}`) empezaron como dos datos deliberadamente separados — ver commit history/README de esta sección en versiones previas. En la práctica eso confundía: marcar una película vista desde la filmografía de un actor no la mostraba vista en búsqueda/watchlist/Connections, y viceversa. Revertido a propósito: `seen` es ahora la única fuente de verdad de "vi esta película", en toda la app. El checkbox de `MovieItem.tsx` (filmografía de una persona) ya no escribe en `watchedMovies` — llama `markMovieSeen`/`unmarkMovieSeen` como cualquier otro `MovieSeenButton`. El % de avance de una filmografía (`Filmography.tsx`, `Dashboard.tsx`, `WrappedStats.tsx`) se calcula intersectando la lista de películas de la persona (ya se tiene, del mismo fetch de TMDB) contra el set global de `seen` — ya no hay un listener Firestore por persona para esto, solo uno global (`subscribeToSeenMovies`) más liviano.
  - **Migración, no reset**: los `watchedMovies` ya existentes de cada usuario no se pierden. `migrateWatchedToSeen` (`firestore.ts`) hace un backfill único por persona: lee los ids de `watchedMovies` (`getLegacyWatchedIds`), y por cada uno que falte en `seen`, escribe el doc completo usando la metadata que ya está en memoria (de `fetchPersonData`/el prop `movies`) — sin pegarle a TMDB de nuevo. Idempotente (filtra contra el `seen` actual antes de escribir) y se dispara solo, la primera vez que `Filmography.tsx` o `Dashboard.tsx` cargan esa persona — no hace falta ningún script ni acción manual del usuario.
  - `watchedMovies` queda en `firestore.rules` solo para poder seguir leyendo/limpiando datos viejos — la app ya no escribe ahí.
- **`/watched` (2026-09-14)**: página con _todo_ lo marcado como visto, sin importar desde dónde se marcó — `WatchedPage.tsx`, fuente `subscribeToSeenMoviesFull` (a diferencia de `subscribeToSeenMovies`, que solo da los ids). Agregó `genreIds?` a `SeenMovie` (mismo patrón opcional-y-backfilleado que `WatchlistMovie.genreIds`, `setSeenGenres` + `mapWithConcurrency`) para el filtro por género. Dos agrupaciones: por año (igual que `Filmography.tsx`/`WatchlistPage.tsx`) y por persona — esta última cruza cada película vista contra la filmografía de cada persona seguida (`fetchPersonData` con concurrencia acotada, se dispara solo al cambiar a esa vista, no de entrada) para saber a quién atribuirla; lo que no matchea ninguna filmografía seguida (marcado desde búsqueda, o de alguien que ya no sigue) cae en una sección "Not part of a followed filmography" en vez de perderse.
...
- **Concurrencia acotada para fetches "uno por persona/película" (2026-09-14)**: `Dashboard.tsx`, `WrappedStats.tsx` y `ConnectionsPage.tsx` disparaban `fetchPersonData` para cada persona seguida sin ningún límite (todas a la vez, `people.forEach`) o secuencial (`for...of` con `await`, una por una); `WatchlistPage.tsx` backfillaba `genreIds` igual, secuencial. Con una cuenta real de 50 personas seguidas / 122 películas en watchlist, esto generaba: (a) ráfagas de 50 requests simultáneos que superaban el rate limit del endpoint (30/60s), tirando 429 silenciosos en ~20 de ellos — esas personas quedaban con stats en blanco sin reintento; (b) el caso secuencial, aunque no rompía el límite, escalaba el tiempo de carga linealmente con la cantidad de personas/películas en vez de mantenerse casi constante. `src/lib/concurrency.ts` (`mapWithConcurrency`) reemplaza ambos patrones por un pool de 6 en paralelo — ni ráfaga total ni fila secuencial. Cada tarjeta sigue llenándose de forma progresiva apenas resuelve su propio request (nada nuevo ahí, ya renderizaba por-tarjeta). Subido además el rate limit de `/api/person` y `/api/movie` de 30 a 120 req/60s — 30 era para proteger contra scraping anónimo, pero terminaba bloqueando el uso normal de la propia cuenta del usuario con muchas personas seguidas; los endpoints de búsqueda (`search-person`, `search-movie`, `movie-similar`, `person-images`) se dejaron en 30, ahí sí tiene sentido ser más estricto (typeahead).
...
- **Compartir badge como imagen**: `src/lib/shareBadgeImage.ts` dibuja el badge en un `<canvas>` (Canvas API nativo, sin dependencia nueva) y lo pasa a `navigator.share` con fallback a descarga si el navegador no soporta compartir archivos. Look fijo (fondo oscuro + acento del logo) independiente del tema claro/oscuro del usuario — es un asset de marca para compartir afuera de la app, no parte de la UI.
- **Alertas de estreno (`users/{userId}/notifications`)**: cron diario (`vercel.json`, `0 13 * * *`) contra `/api/cron/check-releases`, que usa el **Admin SDK** (`src/lib/firebase/admin.ts`, `FIREBASE_SERVICE_ACCOUNT`) porque necesita leer `followedPeople` de _todos_ los usuarios — algo que el SDK de cliente no puede hacer bajo `firestore.rules` (que están, correctamente, aisladas por `uid`). Por cada persona seguida reutiliza `getFilmography` (mismo helper que ya usa `/api/person/{id}`) y filtra películas con `releaseDate` en los últimos 2 días. El id del doc de notificación es determinístico (`release-{personId}-{movieId}`) y se escribe con `create()` (falla si ya existe, se ignora el error) — eso es lo que evita duplicados, no la ventana de 2 días, que solo está para tolerar un cron run tardío o perdido sin lógica extra de "ya revisado". El endpoint exige `Authorization: Bearer $CRON_SECRET` — sin ese secreto configurado en Vercel, cualquiera podría dispararlo y gastar cuota de TMDB además de escribir en Firestore de todos los usuarios. **Bloqueado en el usuario**: `FIREBASE_SERVICE_ACCOUNT` (JSON de una service account, Firebase Console → Project settings → Service accounts) y `CRON_SECRET` deben cargarse a mano en las env vars de Vercel — no se pueden generar ni setear desde acá. La UI es una campanita (`NotificationBell.tsx`) en el nav, visible solo con sesión iniciada; abrir el dropdown marca como leídas las notificaciones no leídas.
